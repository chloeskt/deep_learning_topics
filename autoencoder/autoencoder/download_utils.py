...
def download_dataset(
    url, data_dir, dataset_zip_name, force_download=False, verbose=False
):
    """
    Download dataset
    ...
    if not os.path.exists(data_dir) or not os.listdir(data_dir) or force_download:
        ...
        if verbose:
            print("Downloading", dataset_zip_name)
        data_file = download_url(url, data_dir, dataset_zip_name)
        if verbose:
            print("Extracting", dataset_zip_name)
        extract_archive(data_file, remove_finished=True)
        if verbose:
            print("Dataset successfully downloaded! Stored under:", data_dir)
    else:
        if verbose:
            print(
                "Found dataset folder. Skipped downloading. If you face issues, "
                "please re-download the dataset using\n'--force_download=True'"
            )
