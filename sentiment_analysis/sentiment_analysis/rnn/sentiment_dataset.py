...
def download_data(url, data_dir="./data"):
...
def create_dummy_data(base_dir, sample_size=3, max_len=20, min_len=5):
    ...
    data = random.sample(train_data1, sample_size) + random.sample(
        train_data0, sample_size
    )
    return data
...
def collate(batch):
    ...
    return {"data": data, "label": label, "lengths": lengths}
